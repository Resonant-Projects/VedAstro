...
namespace VedAstro.Library
{
    ...
    public partial class CalculateHoroscope
    {
        ...
        {
            var sunSign = Calculate.PlanetRasiD1Sign(Sun, birthTime).GetSignName();
            var bindus = Calculate.PlanetAshtakvargaBindu(Sun, sunSign, birthTime);
            var sunIsLagnaLord = Calculate.LordOfHouse(House1, birthTime) == Sun;
            var sunIsInLagna = Calculate.HousePlanetOccupiesBasedOnSign(Sun, birthTime) == House1;
            return CalculatorResult.New(sunIsLagnaLord && sunIsInLagna && bindus >= 5, new[] { Sun }, birthTime);
        }
        ...
        public static CalculatorResult SunAshtakavargaYoga10(Time birthTime)
        {
            var sunSign = Calculate.PlanetRasiD1Sign(Sun, birthTime).GetSignName();
            var bindus = Calculate.PlanetAshtakvargaBindu(Sun, sunSign, birthTime);
            var sunInSixth = Calculate.HousePlanetOccupiesBasedOnSign(Sun, birthTime) == House6;
            var jupiterAspectsSun = Calculate.IsPlanetAspectedByPlanet(Sun, Jupiter, birthTime);
            return CalculatorResult.New(sunInSixth && bindus >= 4 && jupiterAspectsSun,
                new[] { Sun, Jupiter }, birthTime);
        }
        ...
        public static CalculatorResult MarsAshtakavargaYoga7(Time birthTime)
        {
            ...
            var bindus = Calculate.PlanetAshtakvargaBindu(Mars, marsSign, birthTime);
            var marsHouse = Calculate.HousePlanetOccupiesBasedOnSign(Mars, birthTime);
            var fromMoon = Calculate.SignDistanceFromPlanetToPlanet(Moon, Mars, birthTime);
            var inRequiredPlace = marsHouse == House1 || fromMoon is 1 or 9 or 10;
            var dignified = Calculate.IsPlanetInOwnSign(Mars, birthTime) || Calculate.IsPlanetExaltedSign(Mars, birthTime);
            return CalculatorResult.New(bindus >= 4 && inRequiredPlace && dignified, new[] { Mars, Moon }, birthTime);
        }
        ...
        {
            var ketuSign = Calculate.PlanetRasiD1Sign(Ketu, birthTime).GetSignName();
            var bindus = Calculate.PlanetAshtakvargaBindu(Mercury, ketuSign, birthTime);
            var ketuInFifth = Calculate.HousePlanetOccupiesBasedOnSign(Ketu, birthTime) == House5;
            var fifthLord = Calculate.LordOfHouse(House5, birthTime);
            var ketuWithFifthLord = Calculate.IsPlanetConjunctWithPlanet(Ketu, fifthLord, birthTime);
            return CalculatorResult.New(bindus >= 3 && (ketuInFifth || ketuWithFifthLord),
                new[] { Mercury, Ketu, fifthLord }, birthTime);
        }
        ...
        public static CalculatorResult MercuryAshtakavargaYoga12A(Time birthTime)
        {
            ...
            var lordSign = Calculate.PlanetRasiD1Sign(mercurySignLord, birthTime).GetSignName();
            var mercuryBindus = Calculate.PlanetAshtakvargaBindu(Mercury, mercurySign, birthTime);
            var lordBindus = Calculate.PlanetAshtakvargaBindu(Mercury, lordSign, birthTime);
            var lordInTrineOrQuadrant = Calculate.IsPlanetInTrikona(mercurySignLord, birthTime) ||
                                        Calculate.IsPlanetInKendra(mercurySignLord, birthTime);
            return CalculatorResult.New(lordInTrineOrQuadrant && lordBindus > mercuryBindus,
                new[] { Mercury, mercurySignLord }, birthTime);
        }
