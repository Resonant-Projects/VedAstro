...
namespace VedAstro.Library
{
    ...
    public partial class Calculate
    {
        ...
        /// D2
        /// </summary>
        public static ZodiacSign PlanetHoraSigns(PlanetName planetName, Time time) => Calculate.HoraSignName(Calculate.PlanetZodiacSign(planetName, time));
        ...
        public static Time TajikaDateForYear(Time birthTime, int scanYear)
        {
            ...
            Time _tajikaDateForYear()
            {
                var elapsedYears = scanYear - birthTime.StdYear();
                if (elapsedYears < 0)
                {
                    throw new ArgumentOutOfRangeException(nameof(scanYear),
                        "Tajika year cannot precede the birth year.");
                }

                // Start at the modern sidereal-year estimate, then converge on
                // the actual sidereal solar return. The Sun moves about
                // 0.9856 degrees/day; recomputing the correction also absorbs
                // the small variation caused by Earth's elliptical orbit.
        ...
        public static LunarMonth LunarMonth(Time inputTime, bool ignoreLeapMonth = false)
        {
            ...
            //get sign as number
            // At the coarse conjunction scan boundary the Moon can already
            // have crossed a sign while the Sun has not. Lunar month naming
            // is based on the solar sign containing the conjunction.
            var thisMonthSign = (int)Calculate.SunSign(lastNewMoonRaw).GetSignName();
            var nextMonthSign = (int)Calculate.SunSign(nextNewMoon).GetSignName();
        ...
        public static Angle SunMoonConjunctionAngle(Time ccc)
        {
            ...
            var difference = moonLong.GetDifference(sunLong).Normalize360().TotalDegrees;
            return Angle.FromDegrees(Math.Min(difference, 360d - difference));
        }
        ...
        {
            var svgString = new NorthChartFactory(time, ChartType.RasiD1).SVGChart;
        ...
        public static double PlanetIshtaKashtaScoreDegree(PlanetName planet, Time birthTime)
        {
            ...
            var total = ishtaScore + kashtaScore;
            if (total == 0) { return 0; }

            // Normalize the signed balance to the documented -4..+4 range.
            return Math.Round(4 * (ishtaScore - kashtaScore) / total, 3);
        }
        ...
        public static Angle HouseJunctionPoint(Angle previousHouse, Angle nextHouse)
        {
            ...
            Angle _getHouseJunctionPoint()
            {
                ...
                if (nextHouse < previousHouse)
                {
                    ...
                    // Divide on the unwrapped circle and normalize the result.
                    // Subtracting 360 unconditionally produced negative house
                    // junctions when the midpoint itself was below 360°.
                    junctionPoint = longitudeSum.Divide(2).Normalize360();
                }
                else
                {
        ...
        public static Shashtiamsa SunChestaBala(Time inputTime)
        {
            ...
            var sunSayana = Calculate.PlanetSayanaLongitude(Sun, inputTime);

            //The result is Sun's Chesta kendra
            var chestaKendra = (sunSayana + Angle.Degrees90).Normalize360().TotalDegrees;
